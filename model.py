...
from helper_modules import PointWiseFeedForward, T2VSASRec
from utils import get_nums

class LightT2VSAS(LightningModule):
    def __init__(self, config):
        super().__init__()
        item_num, user_num = get_nums(config)
        self.model = T2VSASRec(item_num, config)
    ...
    def training_step(self, batch, batch_idx):
        log_seqs, time_seqs, item_indices = batch["input_seq"], batch["time_seq"], batch["sample_seq"]
        #print(seqs.get_device())
        log_feats = self.model(log_seqs, time_seqs)
        #print(log_feats.shape)
        final_feat = log_feats[:, -1, :]  #ONLY THE FINAL EMBEDDING - what we change in Dense All Action??
        item_embs = self.model.item_emb(item_indices).to(self.device)
        
        sample_logits = (item_embs @ final_feat.unsqueeze(-1)).squeeze(-1)
